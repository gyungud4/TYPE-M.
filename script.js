...
function render() {
    ...
    // 답변 전에는 0 / 20 등으로 표시되도록 qIdx를 그대로 노출 (단, 첫 화면 렌더링 시 0 처리)
    document.getElementById('quiz-info').innerText = `${qIdx} / ${qArr.length}`;

    // 마지막 질문까지 완료된 후에는 텍스트 노출 안함 (에러 방지)
    if (qIdx < qArr.length) {
...
// 1. 1단계 종료 시 호출되는 함수 수정
function finishPhase1() {
    // 유형 결정
    finalType = typeScore.gen >= typeScore.skl ? 'gen' : 'skl';
    
    // 퀴즈 섹션 숨기고 중간 결과 섹션 보이기
    document.getElementById('quiz-section').classList.add('hidden');
...
// 2. '세부 분석 하러 가기' 버튼 클릭 시 호출될 함수 추가
function startPhase2() {
    phase = 2;
    qIdx = 0;
    
    // 중간 결과 섹션 숨기고 다시 퀴즈 섹션 보이기
    document.getElementById('mid-result-section').classList.add('hidden');
    document.getElementById('quiz-section').classList.remove('hidden');
    
    document.getElementById('step-badge').innerText = "STEP 2: 세부 분야 추천";
    render(); // 2단계 첫 번째 질문 렌더링
}